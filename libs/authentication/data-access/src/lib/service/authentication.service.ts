...
export class AuthenticationService {
  ...
  constructor(private httpClient: HttpClient, @Inject(RECIPTOR_API_URL) private apiUrl: string) {
    this.loggedIn$ = new BehaviorSubject<boolean>(AuthenticationService.hasValidToken())
  }
  ...
  private static hasValidToken(): boolean {
    ...
    const isExpired = expires !== null && expires !== undefined && moment() .isBefore(moment(parseInt(expires)));
